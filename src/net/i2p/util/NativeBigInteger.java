	...
	*/
    ...
    private static final boolean loadFromResource(boolean optimized) {
        ...
        try {
        	...
        	Logger.error(NativeBigInteger.class, "NativeBigInteger : Can't load from " + System.getProperty("java.io.tmpdir"));
        	System.err.println("Can't load from " + System.getProperty("java.io.tmpdir"));
        	if(tryLoadResource(new File("jbigi-lib.tmp"), resource))
        		return true;
        } catch(Exception fnf) {
        	Logger.error(NativeBigInteger.class, "NativeBigInteger : Error reading jbigi resource");
        	System.err.println("Error reading jbigi resource");
        } catch(UnsatisfiedLinkError ule) {
        	Logger.error(NativeBigInteger.class, "NativeBigInteger : Library " + resourceName + " is not appropriate for this system.");
